...
async function init(){
  // Detect base URL (Apps Script web app) when served; otherwise expect same origin
  state.baseUrl = window.WEB_APP_URL || '';
  if (!state.baseUrl) {
    // Default to relative; the Apps Script will read ?path=.. param
    state.baseUrl = '';
  }
...
function renderDashboard(){
  ...
  // Fetch today's trades to compute PnL and hit rate
...
// API helpers (Apps Script: use ?path=..)
async function apiGet(path){
  const url = path.startsWith('/api/') ? `?path=${encodeURIComponent(path)}` : path;
...
// Client-side planning (mirror to server)
